...
def plot_cm(cm):
    # classes = ['big_flake','granular','mesh_tub', 'short_tub', 'sml_platelet', 'sperical', 'tubular']
    classes = ['granular', 'platelet', 'spherical_hollow', 'spherical_onion', 'tubular_balls']
    print(cm.shape)
    df_cm = pd.DataFrame(cm, index=classes, columns=classes)
    ax = sns.heatmap(df_cm, annot = True, fmt='.2f', linewidth=.5)
        ...
        if mode == "test":
            ...
            with open("output.txt", "a") as f:
                for i in range(pred_cls.shape[0]):
                    print(file_name[i], pred_cls[i].cpu().numpy(), y[i].cpu().numpy(), file=f)
            # cm = confusion_matrix(y.cpu().numpy(), pred_cls.cpu().numpy(), normalize = 'true')
            # plot_cm(cm)
